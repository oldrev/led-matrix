...
static bool checkreturn default_extension_decoder(
    pb_istream_t* stream, pb_extension_t* extension, uint32_t tag, pb_wire_type_t wire_type);
static bool checkreturn decode_extension(
    pb_istream_t* stream, uint32_t tag, pb_wire_type_t wire_type, pb_field_iter_t* iter);
...
static const pb_decoder_t PB_DECODERS[PB_LTYPES_COUNT]
    = { &pb_dec_bool, &pb_dec_varint, &pb_dec_uvarint, &pb_dec_svarint, &pb_dec_fixed32, &pb_dec_fixed64,
...
bool pb_decode_fixed32(pb_istream_t* stream, void* dest)
{
    ...
    *(uint32_t*)dest = ((uint32_t)bytes[0] << 0) | ((uint32_t)bytes[1] << 8) | ((uint32_t)bytes[2] << 16)
        | ((uint32_t)bytes[3] << 24);
    return true;
}
